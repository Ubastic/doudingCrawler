...
var downFreeFile = function (urlbf, callback) {
	console.log("urlbf" + urlbf);
	if (typeof(urlbf) != "string")
	{
		console.log("type: " + typeof(urlbf));
		callback(null);
	}
	else
	{
		console.log("nomarl type: " + typeof(urlbf));
		...
		var key = obj.key;
		var getUrl = urlbf;
		var dirpath = process.cwd() + "\\File\\" + subType
		var path = dirpath + "\\" + fname;

		var dirExists = function (done) {
			fs.exists(dirpath, function (args) {
				if (args == true)
				{
					done(null);
				} else 
				{
					fs.mkdir(dirpath, [777], function(args){
						console.log(dirpath);
						done(null);
					})
				}
		...
		var taskFile = [dirExists, fileExists];
		async.series(taskFile, function (err) {
			if(err) console.error(err.stack);
			console.log("Now DownloadFile");
			callback();
		});
	}	
}
...
exports.getFreeFile = function (documentId, sType, callback) {
